...
SELECT Restaurant_name, Restaurant_address, Price_range
FROM RESTAURANTS AS R
JOIN TRAVEL_ATTRACTIONS AS TA ON R.Attraction_ID = TA.Attraction_ID
JOIN CITY AS C ON TA.City_ID = C.City_ID
JOIN DESTINATION AS D ON TA.City_ID = D.Destination_ID
...
SELECT U.User_Name, U.Is_Preferred, U.Ranking
FROM AUTHORIZED_MEMBER AS U
JOIN IMAGES AS I ON U.Member_ID = I.Member_ID
GROUP BY U.Member_ID
ORDER BY COUNT(I.Image_ID) DESC LIMIT 1;
...
FROM PLANNED_ATTRACTIONS P1 -- Gets PLANNED_ATTRACTIONS
JOIN TRAVEL_ATTRACTIONS TA1 ON P1.Attraction_ID = TA1.Attraction_ID  -- Joins PLANNED_ATTRACTIONS and TRAVEL_ATTRACIONS
JOIN CITY C1 ON TA1.City_ID = C1.City_ID  -- Joins TRAVEL_ATTRACTIONS and CITY
JOIN STATE S1 ON C1.State_ID = S1.State_ID  -- Joins CITY and STATE
WHERE S1.Country_Name <> 'USA' AND EXISTS (SELECT * -- Makes sure S1 is not in the USA and compares to another date
			  FROM PLANNED_ATTRACTIONS P2   -- GETS PLANNED_ATTRACTIONS
			 RIGHT JOIN 
...
RIGHT JOIN TRAVEL_ATTRACTIONS ON BUSINESS_OWNER.Owner_ID = TRAVEL_ATTRACTIONS.Owner_ID -- Joins TRAVEL_ATTRACTIONS and BUSINESS_OWNER
LEFT JOIN RESTAURANTS ON TRAVEL_ATTRACTIONS.Attraction_ID = RESTAURANTS.Attraction_ID -- Joins RESTAURANTS and TRAVEL_ATTRACTIONS
